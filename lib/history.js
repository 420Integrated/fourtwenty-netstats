...
History.prototype.getSmokeLimit = function()
{
	var smokeLimitHistory = _( this._items )
		.sortByOrder( 'height', false )
		// .filter(function (item)
		// {
		// 	return item.block.trusted;
		...
		.map(function (item)
		{
			return item.block.smokeLimit;
		})
		.value();

	return smokeLimitHistory;
}
...
History.prototype.getSmokeSpending = function()
{
	var smokeSpending = _( this._items )
		.sortByOrder( 'height', false )
		.filter(function (item)
		{
			return item.block.trusted;
		...
		.map(function (item)
		{
			return item.block.smokeUsed;
		})
		.value();

	return smokeSpending;
}
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		var chartHistory = _( this._items )
			...
			.map(function (item)
			{
				return {
					...
					smokeSpending: item.block.smokeUsed,
					smokeLimit: item.block.smokeLimit,
		...
		this._callback(null, {
			...
			smokeSpending : _.pluck( chartHistory, 'smokeSpending' ),
			smokeLimit : _.pluck( chartHistory, 'smokeLimit' ),
